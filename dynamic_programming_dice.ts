...
const points : number[] = Array(100); // The point value of the tiles
const expectedValue : number[] = Array(101).fill(0); // The expected value of play starting from a specific tile
const probabilities : ProbabilityList[] = Array(101); // The probabilities of different scores starting from a specific tile
...
for (let i = expectedValue.length - 1; i >= 0; i--) {
    if (i === 100) {
        ...
        for (let j = 1; j <= 6; j++) {
            if (i + j <= 100) {
                ep = ep + (1 / 6) * expectedValue[i + j];
                for (const probability of probabilities[i + j].list) {
                    const newProbability = new Probability(currentPoint + probability.value, (1 / 6) * probability.probability);
                    probabilityList.insert(newProbability);
                }
            } else {
                const newProbability = new Probability(currentPoint, 1/6);
                probabilityList.insert(newProbability);
            }
        }
